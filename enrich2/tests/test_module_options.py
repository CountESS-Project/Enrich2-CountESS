...
class TestBaseOption(TestCase):
    ...
    def test_base_option_equal(self):
        option_1 = BaseOption(
            'TestOption', 'test_option', dtype=str,
            default='default string', hidden=True
        )
        option_2 = BaseOption(
            'TestOption', 'test_option', dtype=str,
            default='default string', hidden=True
    ...
    def test_base_option_not_equal(self):
        ...
        option_2 = BaseOption(
            'TestOption2', 'test_option2', dtype=str,
            default='default string', hidden=False
        )
    ...
    def test_base_option_bad_type_varname(self):
        with self.assertRaises(TypeError):
            option = BaseOption(
                'TestOption', 1234, dtype=str,
                default='default string', hidden=True
            )
        with self.assertRaises(ValueError):
            option = BaseOption(
                'TestOption', '', dtype=str,
                default='default string', hidden=True
            )
    ...
    def test_base_option_bad_type_hidden(self):
        with self.assertRaises(TypeError):
            option = BaseOption(
                'TestOption', 1234, dtype=str,
                default='default string', hidden={}
            )

    def test_base_option_dtype_mismatch(self):
        with self.assertRaises(TypeError):
            option = BaseOption(
                'TestOption', 'test_option', dtype=int,
                default='default string', hidden=True
            )
        with self.assertRaises(TypeError):
            option = BaseOption(
                'TestOption', 'test_option', dtype=None,
                default='default string', hidden=True
            )
...
class TestOptions(TestCase):
    ...
    def test_get_visible_options(self):
        ...
        expected = sorted(
            [o.varname for o in options.get_visible_options()])
        result = ['logr_method']
        self.assertEqual(expected, result)
    ...
    def test_get_hidden_options(self):
        ...
        expected = sorted(
            [o.varname for o in options.get_hidden_options()])
        result = ['weighted']
        self.assertEqual(expected, result)
...
class TestUtilityFunctions(TestCase):
    ...
    def setUp(self):
        self.cfg = {
            'random_option': 1, 'logr_method': 'full', 'weighted': False
        }
    ...
    def test_get_unused_options_ls(self):
        ...
        result = get_unused_options_ls(
            self.cfg, [self.options_1, self.options_2])
        self.assertEqual(result, expected)
