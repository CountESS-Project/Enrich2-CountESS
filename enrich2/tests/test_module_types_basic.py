    ...
    def test_error_invalid_plugin_file(self):
        cfg = {
            SCORER_PATH: os.path.join(
                self.plugin_dir, 'bad_scorer_incomplete.py'),
...
class BarcodeConfigTest(TestCase):
    ...
    def test_error_mapfile_not_exist(self):
        cfg = {
            BARCODE_MAP_FILE: os.path.join(self.data_path, 'i_dont_exist')
        }
        with self.assertRaises(IOError):
            BarcodeConfiguration(cfg).validate()
    ...
    def test_error_mapfile_invalid_ext(self):
        cfg = {
            BARCODE_MAP_FILE: os.path.join(self.data_path, 'polyA_t0.fq')
        }
        with self.assertRaises(IOError):
            BarcodeConfiguration(cfg).validate()
