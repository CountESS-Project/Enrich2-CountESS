...
from ..base.constants import AA_LIST, AA_LABEL_GROUPS, NT_LIST
from ..plotting.plots import plot_cmaps, plot_colors
...
def parse_aa_list(fname):
    ...
    pos = -1
    label_groups = list()
    for label in list(groups_dict.keys()):
...
def sfmap_axes(df, ax, style, wt, df_se=None, tall=False, colors=None,
    ...
    *df_se* contains the standard error of each item in *df*
    ...
    if missing_color is not None:
        ...
        cmap.set_bad(plot_colors['missing'], 1.)
    ...
    if show_variants:
        if tall:
            ...
            # no amino acid properties, just add padding
                ...
                wt_ax.set_ylim(0, len(df.index) + 1.)
        else:
