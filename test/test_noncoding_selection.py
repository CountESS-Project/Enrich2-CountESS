...
def suite():
    suite = unittest.TestLoader().loadTestsFromTestCase(TestNoncodingSelection)
    return suite
...
class TestNoncodingSelection(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cfg_file = os.path.join(os.path.dirname(__file__), "test_files", "config", "polyA_noncoding.json")
        try:
    ...
    def test_raw_counts_sorted(self):
        result = pd.DataFrame.from_csv(os.path.join(os.path.dirname(__file__), "test_files", "result", "polyA_noncoding_variant_counts.tsv"), sep='\t').astype(float)
        # order in h5 matters
