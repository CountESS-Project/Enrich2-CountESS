...
from enrich2.base.dataframe import single_mutations_to_tuples
from enrich2.base.dataframe import single_mutation_index, filter_coding_index
from enrich2.base.dataframe import SingleMut

from enrich2.sequence.wildtype import WildTypeSequence
...
class TestUtilitiesDataframe(unittest.TestCase):
    ...
    def test_single_mutation_index(self):
        ...
        self.assertTrue(all(single_mutation_index(
            pd.Index(["c.76A>C (p.Ile26Leu)"])) == pd.Index(
            ["c.76A>C (p.Ile26Leu)"])))
        self.assertTrue(all(single_mutation_index(pd.Index(
            ["c.76A>C (p.Ile26Leu)",
             "c.76A>C (p.Ile26Leu), c.78C>T (p.Ile26Leu)"])) == pd.Index(
            ["c.76A>C (p.Ile26Leu)"])))
        self.assertTrue(all(single_mutation_index(pd.Index(
            ["c.76A>C (p.Ile26Leu)", "c.78C>T (p.Ile26Leu)"])) == pd.Index(
        ...
        self.assertTrue(all(single_mutation_index(pd.Index(
            ["c.76A>C (p.Ile26Leu)",
             "c.76A>C (p.Ile26Leu), c.78C>T (p.Ile26Leu)",
             "c.78C>T (p.Ile26Leu)"])) == pd.Index(
            ["c.76A>C (p.Ile26Leu)", "c.78C>T (p.Ile26Leu)"])))

        # order matters
        self.assertFalse(all(single_mutation_index(pd.Index(
            ["c.78C>T (p.Ile26Leu)", "c.76A>C (p.Ile26Leu)"])) == pd.Index(
            ["c.76A>C (p.Ile26Leu)", "c.78C>T (p.Ile26Leu)"])))

        # empty indices
        self.assertEqual(len(single_mutation_index(pd.Index([]))), 0)
        self.assertEqual(len(single_mutation_index(
            pd.Index(["c.76A>C (p.Ile26Leu), c.78C>T (p.Ile26Leu)"]))), 0)

    def test_filter_coding_index(self):
        # test cases use all() because testing equality of indices returns a vector of booleans

        # test removal of '???' variants
        self.assertTrue(all(filter_coding_index(
            pd.Index(["c.76A>C (p.Ile26Leu)"])) == pd.Index(
            ["c.76A>C (p.Ile26Leu)"])))
        self.assertTrue(all(filter_coding_index(pd.Index(
            ["c.76A>C (p.Ile26Leu)", "c.76A>N (p.Ile26???)"])) == pd.Index(
            ["c.76A>C (p.Ile26Leu)"])))
        self.assertTrue(all(filter_coding_index(pd.Index(
            ["c.76A>N (p.Ile26???)", "c.76A>C (p.Ile26Leu)"])) == pd.Index(
            ["c.76A>C (p.Ile26Leu)"])))
        self.assertTrue(all(filter_coding_index(pd.Index(
            ["c.76A>C (p.Ile26Leu)", "c.78C>T (p.Ile26Leu)"])) == pd.Index(
            ["c.76A>C (p.Ile26Leu)", "c.78C>T (p.Ile26Leu)"])))
        self.assertTrue(all(filter_coding_index(pd.Index(
            ["c.76A>C (p.Ile26Leu)", "c.76A>N (p.Ile26???)",
             "c.78C>T (p.Ile26Leu)"])) == pd.Index(
            ["c.76A>C (p.Ile26Leu)", "c.78C>T (p.Ile26Leu)"])))
    ...
    def test_single_mutations_to_tuples(self):
        ...
        coding_tup_1 = SingleMut(pre='A', post='C', pos=76,
                                 key='c.76A>C (p.Ile26Leu)')
        coding_tup_2 = SingleMut(pre='C', post='T', pos=78,
                                 key='c.78C>T (p.Ile26Leu)')
        self.assertEqual(
    ...
    def test_fill_position_gaps(self):
        ...
        self.assertSequenceEqual(fill_position_gaps([1, 5], 4),
                                 [1, 2, 3, 4, 5])
        self.assertSequenceEqual(fill_position_gaps([1, 1, 5], 4),
                                 [1, 2, 3, 4, 5])
        self.assertSequenceEqual(fill_position_gaps([5, 1], 4),
                                 [1, 2, 3, 4, 5])
        self.assertSequenceEqual(fill_position_gaps([5, 1, 5], 4),
                                 [1, 2, 3, 4, 5])
        self.assertSequenceEqual(fill_position_gaps([1, 5, 15], 4),
                                 [1, 2, 3, 4, 5, 15])
        self.assertSequenceEqual(fill_position_gaps([15, 5, 1], 4),
                                 [1, 2, 3, 4, 5, 15])
        self.assertSequenceEqual(fill_position_gaps([15, 1, 5], 4),
                                 [1, 2, 3, 4, 5, 15])
        self.assertSequenceEqual(fill_position_gaps([15, 17, 15], 4),
                                 [15, 16, 17])
        self.assertSequenceEqual(fill_position_gaps([1, 1, 1, 1], 1),
                                 [1])
    ...
    def test_singleton_dataframe_offset(self):
        ...
        values = pd.Series(
            data=[1, 2, 3],
            index=['c.4A>T (p.Ile3Leu)', 'c.5A>C (p.Ile5Leu)', '_wt']
        )
    ...
    def test_singleton_dataframe_noncoding(self):
        ...
        values = pd.Series(
            data=[1, 2, 3, 4],
            index=['c.1A>G (p.Ile1Leu)', 'c.3A>T (p.Ile3Leu)',
                   'c.5A>C (p.Ile5Leu)', '_wt']
        )
        frame, wt_sequence = singleton_dataframe(
            values, wt, coding=False, plot_wt_score=False)
        self.assertTrue(all(frame == frame))
        ...
        values = pd.Series(
            data=[1, 2, 3],
            index=['n.1A>G', 'n.3A>T', 'n.5A>C, n.6A>C']
        )
        frame, wt_sequence = singleton_dataframe(
            values, wt, coding=False, plot_wt_score=False)
        self.assertTrue(all(frame == frame))
        ...
        values = pd.Series(
            data=[1, 2, 3, 4],
            index=['n.1A>G', 'n.3A>T', 'n.5A>C', 'n.10A>C']
        )
        frame, wt_sequence = singleton_dataframe(
    ...
    def test_singleton_dataframe_coding(self):
        ...
        values = pd.Series(
            data=[1, 2, 3, 4],
            index=['p.Lys1Leu', 'p.Lys3Lys', 'p.Lys5Lys', '_wt']
        )
    ...
    def test_singleton_dataframe_invalid_input(self):
        ...
        with self.assertRaises(ValueError):
            values = pd.Series(
                data=[1, 2, 3, b'4'],
                index=['c.1G>G (p.Ile1Leu)', 'c.3A>T (p.Ile3Leu)',
                       'c.5A>C (p.Ile5Leu)', '_wt']
            )
        ...
        with self.assertRaises(ValueError):
            values = pd.Series(
                data=[1, 2, 3],
                index=['c.1G>G (p.Ile1Leu), c.1G>G (p.Ile1Leu)',
        ...
        with self.assertRaises(KeyError):
            values = pd.Series(
                data=[1, 2, 3, 4],
                index=['n.1A>G (p.Ile1Leu)', 'n.3A>T (p.Ile3Leu)',
                       'n.5A>C (p.Ile5Leu)', '_wt']
            )
